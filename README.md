...
CLI の表示は、日本語環境では日本語、非日本語環境では英語になります。

判定にはターミナルのロケールではなく、保存した HoYoLAB Cookie の `mi18nLang` を優先します。`mi18nLang=ja-jp` なら日本語、それ以外の言語なら英語で表示し、HoYoLAB API の `lang` パラメーターも同じ方針で送ります。

ログイン情報を保存済みの場合は、設定ファイルの `ui_language` / `lang` を使います。初回ログイン前で HoYoLAB の言語情報がまだない場合は、可能な環境では OS の表示言語を参照し、それも分からない場合は英語で表示します。
...
設定ファイルには HoYoLAB API 用の `lang` と CLI 表示用の `ui_language` も保存します。通常はログイン時に取得した HoYoLAB Cookie の `mi18nLang` から自動で決まります。
