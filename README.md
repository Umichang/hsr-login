...
Homebrew 版 Python など PEP 668 に対応した環境では、`python3 -m pip install --user .` が `externally-managed-environment` で失敗します。このリポジトリでは、専用 venv に `pip install` して `hsr-login` コマンドだけを `~/.local/bin` から呼べるようにするインストーラを用意しています。
